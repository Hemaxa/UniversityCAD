...
QWidget* Control::createVariantPopup(LongPressButton* mainBtn, const std::vector<std::pair<QString, int>>& variants, PrimitiveType type) {
    ...
    for (const auto& var : variants) {
        QToolButton* btn = new QToolButton(popup); btn->setIcon(QIcon(var.first)); btn->setIconSize(QSize(24, 24)); btn->setProperty("isIconButton", true);
        int methodIndex = var.second;
        connect(btn, &QToolButton::clicked, this, [this, mainBtn, popup, type, methodIndex, var](){
            popup->close(); mainBtn->setChecked(true); mainBtn->setIcon(QIcon(var.first));
            m_activeSubMethods[type] = methodIndex; emit primitiveTypeSelected(type, methodIndex);
        });
        layout->addWidget(btn);
    }
    return popup;
}
void Control::onPrimitiveToolClicked(int id) { PrimitiveType type = static_cast<PrimitiveType>(id); int method = m_activeSubMethods[type]; emit primitiveTypeSelected(type, method); }
void Control::resetTools() { if (auto* btn = m_primitiveToolsGroup->checkedButton()) { m_primitiveToolsGroup->setExclusive(false); btn->setChecked(false); m_primitiveToolsGroup->setExclusive(true); } emit primitiveTypeSelected(PrimitiveType::Generic, 0); }
...
void Control::updateObjectList(const Scene* scene) {
    ...
    std::vector<Object*> oldSel; for(auto* item : m_objectListWidget->selectedItems()) oldSel.push_back(static_cast<Object*>(item->data(Qt::UserRole).value<void*>()));
    m_objectListWidget->clear();
    if (scene) {
        for (const auto& obj : scene->getPrimitives()) {
            QString name;
            switch(obj->getType()){ case PrimitiveType::Segment: name="Отрезок"; break; case PrimitiveType::Circle: name="Окружность"; break; case PrimitiveType::Arc: name="Дуга"; break; case PrimitiveType::Rectangle: name="Прямоугольник"; break; case PrimitiveType::Ellipse: name="Эллипс"; break; case PrimitiveType::Polygon: name="Полигон"; break; case PrimitiveType::Spline: name="Сплайн"; break; default: name="Объект"; break; }
            QListWidgetItem* item = new QListWidgetItem(QString("%1 %2").arg(name).arg(obj->getID())); item->setData(Qt::UserRole, QVariant::fromValue(static_cast<void*>(obj.get()))); m_objectListWidget->addItem(item);
            for(auto* o : oldSel) if(o == obj.get()) item->setSelected(true);
        }
    }
    m_objectListWidget->blockSignals(false); m_updatingSelection = false;
}
void Control::onSelectionChanged() { if (m_updatingSelection) return; std::vector<Object*> selectedObjects; for (auto* item : m_objectListWidget->selectedItems()) selectedObjects.push_back(static_cast<Object*>(item->data(Qt::UserRole).value<void*>())); emit objectsSelected(selectedObjects); }
void Control::setSelectedObjects(const std::vector<Object*>& objects) { if (m_updatingSelection) return; m_updatingSelection = true; m_objectListWidget->blockSignals(true); m_objectListWidget->clearSelection(); for(int i=0; i<m_objectListWidget->count(); ++i){ auto* item = m_objectListWidget->item(i); Object* obj = static_cast<Object*>(item->data(Qt::UserRole).value<void*>()); for(auto* sel : objects) if(sel == obj) item->setSelected(true); } m_objectListWidget->blockSignals(false); m_updatingSelection = false; }
