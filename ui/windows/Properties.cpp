...
static QDoubleSpinBox* createSpin(double val=0, double min=-100000, double max=100000) {
    ...
    // Отключаем колесико мыши для предотвращения случайных изменений при скролле панели
...
static QLabel* createLbl(const QString& text) {
    ...
    l->setAlignment(Qt::AlignRight | Qt::AlignVCenter); // Выравниваем лейблы по правому краю
    return l;
}
...
// --- УНИФИЦИРОВАННЫЙ ХЕЛПЕР ДЛЯ СЕТКИ ---
QGridLayout* Properties::setupGridLayout(QGroupBox* group) {
    auto* grid = new QGridLayout(group);
    grid->setContentsMargins(8, 12, 8, 8);
    grid->setSpacing(10);
    // Настраиваем растяжение колонок: 0 и 2 (лейблы) - минимум места, 1 и 3 (поля) - максимум
...
// Хелпер для добавления строки свойств (2 колонки)
void Properties::addRow(QGridLayout* layout, int row, QLabel* l1, QWidget* w1, QLabel* l2, QWidget* w2) {
    ...
    if (l2 && w2) {
        ...
        // Если передан только первый элемент, растягиваем его (опционально)
        // layout->addWidget(w1, row, 1, 1, 3);
    }
}
...
QWidget* Properties::createSegmentWidget() {
    ...
    // Заголовки разделов можно реализовать через QLabel с colspan
...
QWidget* Properties::createCircleWidget() {
    ...
    auto* gl = new QVBoxLayout(gb); gl->setSpacing(10);
    ...
    methodLayout->addWidget(m_circleMethodCombo);
    ...
    auto l2p1x = createLbl("X:"); auto l2p1y = createLbl("Y:");
    auto l2p2x = createLbl("X:"); auto l2p2y = createLbl("Y:");
    g2->addWidget(new QLabel("Точка 1:"), 0, 0, 1, 4);
    addRow(g2, 1, l2p1x, m_circ2P1X, l2p1y, m_circ2P1Y);
    g2->addWidget(new QLabel("Точка 2:"), 2, 0, 1, 4);
    addRow(g2, 3, l2p2x, m_circ2P2X, l2p2y, m_circ2P2Y);
    ...
    auto l3p1x = createLbl("X:"); auto l3p1y = createLbl("Y:");
    auto l3p2x = createLbl("X:"); auto l3p2y = createLbl("Y:");
    auto l3p3x = createLbl("X:"); auto l3p3y = createLbl("Y:");
...
QWidget* Properties::createArcWidget() {
    ...
    auto* methodLayout = new QHBoxLayout();
    methodLayout->addWidget(createLbl("Метод:")); methodLayout->addWidget(m_arcMethodCombo);
    gl->addLayout(methodLayout);
    ...
    // Для углов всегда используем лейблы "Нач" и "Угол"
    ...
    auto a3p1x = createLbl("X:"); auto a3p1y = createLbl("Y:");
    auto a3p2x = createLbl("X:"); auto a3p2y = createLbl("Y:");
    auto a3p3x = createLbl("X:"); auto a3p3y = createLbl("Y:");
...
QWidget* Properties::createRectangleWidget() {
    ...
    auto* mh = new QHBoxLayout(); mh->addWidget(createLbl("Метод:")); mh->addWidget(m_rectMethodCombo);
    gl->addLayout(mh);
    ...
    auto r0x1 = createLbl("X:"); auto r0y1 = createLbl("Y:");
    auto r0x2 = createLbl("X:"); auto r0y2 = createLbl("Y:");
    ...
    // Скругление (общий параметр)
    m_rectChamfer = createSpin(0, 0, 50);
    auto* commonLayout = new QHBoxLayout();
    commonLayout->addWidget(createLbl("Скругление:")); commonLayout->addWidget(m_rectChamfer);
    gl->addLayout(commonLayout);
...
QWidget* Properties::createEllipseWidget() {
    ...
    auto* mh = new QHBoxLayout(); mh->addWidget(createLbl("Метод:")); mh->addWidget(m_ellMethodCombo);
    gl->addLayout(mh);
    ...
    m_ell2P1X = createSpin(50); m_ell2P1Y = createSpin(0);
    m_ell2P2X = createSpin(0); m_ell2P2Y = createSpin(30);
    auto e1x = createLbl("X:"); auto e1y = createLbl("Y:");
    auto e1p1x = createLbl("X:"); auto e1p1y = createLbl("Y:");
    auto e1p2x = createLbl("X:"); auto e1p2y = createLbl("Y:");
...
QWidget* Properties::createSplineWidget() {
    ...
    // Для сплайна тоже используем Grid, но он будет динамически наполняться
    auto* pointsContainer = new QWidget();
    m_splinePointsLayout = new QGridLayout(pointsContainer);
    m_splinePointsLayout->setContentsMargins(0,0,0,0);
    m_splinePointsLayout->setSpacing(10);
    m_splinePointsLayout->setColumnStretch(0,0); m_splinePointsLayout->setColumnStretch(1,1);
    m_splinePointsLayout->setColumnStretch(2,0); m_splinePointsLayout->setColumnStretch(3,1);

    gl->addWidget(pointsContainer);

    auto* btnLayout = new QHBoxLayout();
    auto* addBtn = new QPushButton("+ Точка");
    auto* remBtn = new QPushButton("-");
    remBtn->setFixedWidth(30);
...
QGroupBox* Properties::createStyleWidget() {
    ...
    // Стиль располагаем тоже красиво в 2 колонки
...
void Properties::showEditingPropertiesFor(const std::vector<Object*>& objects) {
    ...
    if (objects.size() == 1) {
        m_stack->setCurrentWidget(m_primitiveWidgets[firstType]);
...
void Properties::populateFields(Object* obj) {
    ...
    case PrimitiveType::Circle: {
        ...
        m_circCX->setValue(c->getCenter().getX()); m_circCY->setValue(c->getCenter().getY());
    ...
    case PrimitiveType::Arc: {
        ...
        m_arcCX->setValue(a->getCenter().getX()); m_arcCY->setValue(a->getCenter().getY());
        m_arcR->setValue(a->getRadius());

        // --- ИСПРАВЛЕНИЕ: Конвертация единиц ---
        ...
        m_arcStart->setValue(s); m_arcSpan->setValue(sp);
        break;
    }
    case PrimitiveType::Rectangle: {
        m_rectMethodCombo->setCurrentIndex(1);
        auto* r = static_cast<Rectangle*>(obj);
        m_rect1PX->setValue(r->getTopLeft().getX()); m_rect1PY->setValue(r->getTopLeft().getY());
        m_rect1W->setValue(r->getWidth()); m_rect1H->setValue(r->getHeight());
    ...
    case PrimitiveType::Ellipse: {
        ...
        m_ellCX->setValue(e->getCenter().getX()); m_ellCY->setValue(e->getCenter().getY());
        m_ellRX->setValue(e->getRadiusX()); m_ellRY->setValue(e->getRadiusY());
        break;
    }
    case PrimitiveType::Polygon: {
        auto* p = static_cast<PolygonObj*>(obj);
        m_polyCX->setValue(p->getCenter().getX()); m_polyCY->setValue(p->getCenter().getY());
...
void Properties::updateObjectGeometry(Object* obj) {
    ...
    case PrimitiveType::Circle: {
        ...
        Point c; double r = 0;
        if (method == 0) {
            c = readPoint(m_circCX, m_circCY); r = m_circR->value();
        } else if (method == 1) {
            c = readPoint(m_circCX_D, m_circCY_D); r = m_circD->value() / 2.0;
        } else if (method == 2) {
        ...
        if (auto* circle = dynamic_cast<Circle*>(obj)) {
            circle->setCenter(c); circle->setRadius(r);
        }
        break;
    }
    case PrimitiveType::Arc: {
        ...
        if (method == 0) {
            c = readPoint(m_arcCX, m_arcCY); r = m_arcR->value();
            start = m_arcStart->value(); span = m_arcSpan->value();
    ...
    case PrimitiveType::Rectangle: {
        ...
        if (method == 0) {
            ...
            w = std::abs(p1.getX() - p2.getX()); h = std::abs(p1.getY() - p2.getY());
        } else if (method == 1) {
        ...
        if (auto* rect = dynamic_cast<Rectangle*>(obj)) {
            rect->setTopLeft(tl); rect->setWidth(w); rect->setHeight(h); rect->setCornerRadius(cr);
        }
        break;
    }
    case PrimitiveType::Ellipse: {
        ...
        if (method == 0) {
            ...
            rx = MathUtils::dist(c, p1); ry = MathUtils::dist(c, p2);
        }
